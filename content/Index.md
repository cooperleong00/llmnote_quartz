...
| 入口 | 覆盖范围 | 适合什么时候进入 |
|------|----------|------------------|
| [[MOC - Foundations]] | 数学基础、优化器、Transformer、RL 基础 | 需要补齐前置概念时 |
| [[MOC - Attention]] | Attention、KV Cache、位置编码、高效注意力 | 需要理解 LLM 架构和长上下文机制时 |
| [[MOC - Distributed Training]] | DP/ZeRO/FSDP、TP/PP/EP、显存与训练效率 | 需要理解大模型训练工程时 |
| [[MOC - Inference]] | KV Cache、Paged/Radix Attention、Speculative Decoding、量化 | 需要理解部署与 serving 优化时 |
| [[MOC - RAG]] | 检索、查询优化、重排序、上下文压缩、GraphRAG | 需要构建知识增强系统时 |
| [[MOC - Reinforcement Learning]] | 经典 RL、policy gradient、PPO/GRPO/VAPO、Agentic RL | 需要系统理解 RL 算法演进时 |
| [[MOC - Post-training]] | SFT、RLHF、Direct Alignment、Online RL、OPD、Agentic RL | 需要理解模型对齐和 reasoning model 训练时 |
