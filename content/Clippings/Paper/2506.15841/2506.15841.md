...
arxiv: "2506.15841"
core_method: "[[MEM1]]"
description: 提出 MEM1 框架，通过端到端强化学习实现推理与记忆整合的恒定内存 Agent，在长时域任务中保持性能的同时大幅降低内存开销
tags:
  - agent
  - memory
  - rl
  - long-horizon
  - efficiency
github: https://github.com/MIT-MI/MEM1
updated: 2026-03-06T18:32
---

# MEM1: Learning to Synergize Memory and Reasoning for Efficient Long-Horizon Agents

Zijian Zhou^{∗12} Ao Qu^{∗13} Zhaoxuan Wu^{1} Sunghwan Kim^{4}
Alok Prakash^{1} Daniela Rus^{13} Jinhua Zhao^{13} Bryan Kian Hsiang Low^{13}
Paul Pu Liang^{3}
^{1}Singapore-MIT Alliance for Research and Technology Centre
^{2}National University of Singapore
^{3}MIT
^{4}Yonsei University
[Equal contribution. Correspondence: zhou_zijian@u.nus.edu, qua@mit.edu]
...
TASK: Compare NVIDIA L40S and A100 GPUs, which is more suitable for high-throughput deep learning workloads?
...
Figure 3: (Top): the RL pipeline used to train MEM1. (Bottom left): The evolution of context in MEM1-old  $&lt;\mathrm{IS}&gt;$ ,  $&lt;\mathrm{query}&gt;$ ,  $&lt;\mathrm{info}&gt;$  are cleared as new states enter the context. The mechanism is used in the rollout. (Bottom right): the 2D attention mask used during the objective computation stage. The mask is applied during the forward pass to compute action log-probabilities for the actor model and state value estimates for the critic model. During the policy update stage, the information mask is then applied to the full trajectory, masking out tokens that were not generated by the model itself.
...
We train two versions of MEM1 agent for both long-horizon QA and web navigation. For long-horizon QA, we augment the multi-hop QA dataset from *[24]* that mixes data from both HotpotQA *[59]* and Natural Question *[26]* to form a $2$-objective composite task.
...
Beyond QA tasks, we further evaluate the effectiveness of MEM1 in managing long-horizon interactions in the form of web navigation. We show
...
- [34] Magnus Müller and Gregor Žunič. Browser use: Enable ai to control your browser. https://github.com/browser-use/browser-use, 2024.
...
- [56] Di Wu, Hongwei Wang, Wenhao Yu, Yuwei Zhang, Kai-Wei Chang, and Dong Yu. Long-memeval: Benchmarking chat assistants on long-term interactive memory. In International Conference on Learning Representations (ICLR), 2025.
...
The F1 score computes the harmonic mean between the precision $p$ and recall $r$. In the case of string matching, we split both the predicted answer and the ground truth. For example, if the ground truth is “United States of America”, it is split into a list with lower-case words: “united”, “states”, “of”, “america”. The same works for the predicted answer. Then, denote the number of common words as $c$. Further denote the number of words in the predicted answer as $l$ and the number of words in the ground truth as $g$. Then, precision is calculated as $p\coloneqq c/l$ and recall is calculated as $r\coloneqq c/g$. The F1 score is finally computed as

$\texttt{F1}\coloneqq 2\times\frac{p\times r}{p+r}\,.$
...
Following *[67]*, the dependency metric is defined as the total number of historical tokens on which each generated token effectively depends. Let $T$ denote the total number of interaction steps. For each step $i\in[T]$, let $n_{p}^{(i)}$ be the number of prefix tokens and $n_{o}^{(i)}$ be the number of output tokens generated. The dependency metric is then calculated as

$\texttt{Dependency}\coloneqq\sum_{i\in[T]}\frac{(2n_{o}^{(i)}+n_{p}^{(i)})\times n_{o}^{(i)}}{2}\,.$
...
As detailed in *[24]*, the model is trained on the $1$-objective task with the same dataset as MEM1. Search-R1 also uses exact match as its reward function. In comparison, MEM1 is trained exclusively on $2$-objective tasks.

##### Deep Researcher.

As detailed in *[69]*, the model is trained on $1$-objective task with a curated set from various QA datasets including HotPotQA and Natural Questions. Deep Researcher adopts the F1 score as the reward function.
...
<table><tr><td colspan="2">Algorithm 1 MEM1 Rollout</td></tr><tr><td colspan="2">Require: Task prompt x, policy model πθ, world model W, maximum turn T</td></tr><tr><td colspan="2">Ensure: Final response y</td></tr><tr><td colspan="2">1: Initialize rollout sequence y ← ∅</td></tr><tr><td colspan="2">2: Initialize turn count t ← 0</td></tr><tr><td colspan="2">3: while t &lt; T do</td></tr><tr><td colspan="2">4: Initialize current policy rollout sequence yt ← ∅</td></tr><tr><td colspan="2">5: while True do</td></tr><tr><td colspan="2">6: Generate response token yr ~ πθ(· | x, y + yt)</td></tr><tr><td colspan="2">7: Append yr to rollout sequence yt ← yt + yr</td></tr><tr><td colspan="2">8: if (t = T - 1) and yr ∈ [&lt;/answer&gt;, &lt;eos&gt;] then</td></tr><tr><td colspan="2">9: break // prevent the agent from searching further</td></tr><tr><td colspan="2">10: else if yr ∈ [&lt;/query&gt;, &lt;/answer&gt;, &lt;eos&gt;] then</td></tr><tr><td colspan="2">11: break</td></tr><tr><td colspan="2">12: end if</td></tr><tr><td colspan="2">13: end while</td></tr><tr><td colspan="2">14: y ← yt // all previous context removed.</td></tr><tr><td colspan="2">15: if &lt;query&gt; &lt;/query&gt; detected in yt then</td></tr><tr><td colspan="2">16: Extract search query q ← Parse(yt, &lt;query&gt;, &lt;/query&gt;)</td></tr><tr><td colspan="2">17: Retrieve environment feedback d ← W(q) from local storage, Search engine, HTML, ...</td></tr><tr><td colspan="2">18: HINT ← You have {T - t} turns left.</td></tr><tr><td colspan="2">19: Insert d into rollout y ← y + &lt;info&gt;HINT + d&lt;/info&gt;</td></tr><tr><td colspan="2">20: else if &lt;answer&gt; &lt;/answer&gt; detected in yt then</td></tr><tr><td colspan="2">21: return final generated response y</td></tr><tr><td colspan="2">22: else</td></tr><tr><td colspan="2">23: Mark the sample as invalid</td></tr><tr><td colspan="2">24: end if</td></tr><tr><td colspan="2">25: Increment turn count t ← t + 1</td></tr><tr><td colspan="2">26: end while</td></tr><tr><td colspan="2">27: return final generated response y</td></tr></table>
...
As such, for training efficiency, we do not duplicate the <is> and assign the position ids for the previous trajectory to each <is>. While this modification slightly deviates from the "ideal"</is></is></is>
