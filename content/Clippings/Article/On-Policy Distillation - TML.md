...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/distillation-cover.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/prompt-mobile.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/reinforcement-learning-mobile.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/off-policy-distillation-mobile.svg)

  

Distillation from large model teachers has proven effective in training small models to [follow instructions](https://crfm.stanford.edu/2023/03/13/alpaca.html), [reason on math and science](https://arxiv.org/abs/2506.04178), [extract clinical information](https://arxiv.org/html/2501.00031v1)  from medical notes, and [engage in multi-turn chat dialogues](https://arxiv.org/abs/2305.14233). The distillation datasets used for these and other applications are often open-sourced and published.
...
Another issue observed with off-policy distillation is that the student can learn to imitate the teacher’s style and confidence but [not necessarily its factual accuracy](https://arxiv.org/abs/2305.15717).
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/on-policy-distillation-mobile.svg)
...
Our work with on-policy distillation draws inspiration from [DAGGER](https://arxiv.org/abs/1011.0686), an iterative SFT algorithm that includes teacher evaluations of student-visited states. It is also similar to [process reward modeling,](https://arxiv.org/abs/2305.20050) an RL approach that scores every step in the student model’s chain-of-thought. We extend prior on-policy distillation work by [Agarwal et al,](https://arxiv.org/abs/2306.13649) [Gu et al,](https://arxiv.org/abs/2306.08543) and [the Qwen3 team](https://arxiv.org/abs/2505.09388) . Using the [Tinker training API](https://thinkingmachines.ai/tinker/), we replicate Qwen3’s result of achieving equivalent performance on reasoning benchmarks with on-policy distillation for a fraction the cost of RL.
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/example-kl-illustration.svg)

Example trajectory graded by a teacher model. Tokens in darker red correspond to a higher reverse KL.

Darker colors represent tokens that receive a higher penalty from the teacher model, [Qwen3-235B-A22B-Instruct-2507](https://huggingface.co/Qwen/Qwen3-235B-A22B-Instruct-2507), which solves this problem correctly. We see that it penalizes tokens that start phrases which lead the student astray, intuitively corresponding to important [“forking tokens”](https://arxiv.org/abs/2506.01939) for guiding reasoning. The final answer, though wrong, isn’t penalized — it is entirely predictable conditional on the whole preceding sequence.

### Pseudocode

We implement on-policy distillation on top of the [RL script in Tinker](https://github.com/thinking-machines-lab/tinker-cookbook/blob/main/tinker_cookbook/rl/train.py), which already implements sampling, reward computation, and policy gradient-style training.
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-off-policy-distillation.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-on-policy-distillation-loras.svg)
...
| --- | --- | --- | --- | --- |
| *Initialization: SFT-400K* | 60% | 8.5 × 10 <sup xmlns="http://www.w3.org/1999/xhtml">20</sup> | 3.8 × 10 <sup xmlns="http://www.w3.org/1999/xhtml">20</sup> | – |
| SFT-2M (extrapolated) | ~70% (extrapolated) | 3.4 × 10 <sup xmlns="http://www.w3.org/1999/xhtml">21</sup> | 1.5 × 10 <sup xmlns="http://www.w3.org/1999/xhtml">21</sup> | 1× |
| Reinforcement learning | 68% | \- | \- | ≈1× |
| On-policy distillation | 70% | 8.4 × 10 <sup xmlns="http://www.w3.org/1999/xhtml">19</sup> | 8.2 × 10 <sup xmlns="http://www.w3.org/1999/xhtml">19</sup> | **9-30×** |
...
2. The model exhibits strong **post-training** behavior, ie. instruction following. We will measure this with the commonly-used [IF-eval](https://arxiv.org/abs/2311.07911).
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-personalization-midtrain.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-midtrain-if-eval.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-midtrain-lora.svg)
...
Distillation as a tool for integrating behaviors and knowledge has also been explored for hybrid reasoning models ([Qwen3](https://arxiv.org/abs/2505.09388)) and specialist distillation. As our and [Chen et al.’s](https://arxiv.org/abs/2510.18874) results suggest, on-policy learning can be a crucial tool for augmenting similar distillation-based “model merging” setups.
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-self-distillation.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-self-distillation-multiepoch.svg)
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/svgs/experiment-sft-on-policy.svg)
